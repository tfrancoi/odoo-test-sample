...
class GenerateShiftTemplate(models.TransientModel):
    ...
    day_ids = fields.Many2many('shift.daynumber', relation='template_gen_day_number_rel', column1='wizard_id', column2='day_id')
    planning_id = fields.Many2one('shift.planning', required=True)
    type_id = fields.Many2one('shift.type', default=lambda self: self._context.get('active_id'))
